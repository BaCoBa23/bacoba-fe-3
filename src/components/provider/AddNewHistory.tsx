...
function AddNewHistory({
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const response = await createHistoryProviders(payload);

      if (response.success) {
        toast.success("Đã lập phiếu chi trả thành công!");
        form.reset();
        if (onSuccess) {
          onSuccess();
        }
        setIsOpen(false); // Đóng Dialog
        // Nếu có hàm callback để refresh danh sách NCC, bạn gọi ở đây
      }
    } catch (error: any) {
      console.error("Lỗi khi tạo phiếu chi:", error);
      toast.error(
        error.response?.data?.message || "Không thể tạo phiếu chi trả"
      );
    } finally {
          ...
          >
            ...
            <div className="p-6 bg-chart-2/50 text-chart-2/50-foreground">
              <DialogHeader>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <DialogTitle className="text-sm font-bold text-chart-2/50-foreground">
                      Phiếu Chi Trả Nợ
                    </DialogTitle>
                    <DialogDescription className="text-chart-2/50-foreground/80 flex items-center gap-1">
                      NCC:{" "}
                      <span className="font-medium text-chart-2/50-foreground uppercase">
                        {providerName}
                      </span>
                    </DialogDescription>
                  </div>
            ...
            <div className="px-6 py-4 bg-secondary/50 flex items-center justify-between gap-4">
              ...
              <div className="flex-1 p-3 bg-chart-2/50 rounded-sm shadow-md shadow-chart-2/50/20 text-center">
                <p className="text-[10px] text-chart-2/50-foreground/70 uppercase tracking-wider font-medium mb-1">
                  Số tiền trả
                </p>
                <p className="text-sm font-medium text-chart-2/50-foreground">
                  {formatVND(paidAmount || 0)}
                </p>
              </div>
            ...
            <div className="p-8 space-y-6">
                    ...
                    <FormItem className="col-span-2 sm:col-span-1">
                      ...
                      <FormControl>
                        <div className="relative group">
                          ...
                          />
                          <span className="absolute left-3 top-4 text-muted-foreground font-medium text-lg group-focus-within:text-chart-2/50">
                            ₫
                          </span>
                        </div>
                      </FormControl>
                      ...
                      <FormControl>
                        <div className="relative">
                          <Input
                            type="date"
                            {...field}
                            className="h-14 border-input focus:ring-4 focus:ring-chart-2/50/10 transition-all"
                          />
              ...
              <div className="flex items-start gap-2 p-3 bg-accent rounded-lg border border-border">
                ...
                <p className="text-xs text-accent-foreground/80 leading-relaxed">
                  Lưu ý: Sau khi xác nhận, số nợ của nhà cung cấp sẽ được cập
                  nhật ngay lập tức vào hệ thống.
                </p>
              </div>
            </div>
              ...
              <div className="flex gap-3">
                <DialogClose asChild>
                  <Button
                    variant="ghost"
                    type="button"
                    className="px-6 font-semimedium text-muted-foreground hover:text-foreground"
                  >
                    Hủy
                  </Button>
                </DialogClose>
                <Button
                  type="submit"
                  className="bg-chart-2/50 text-chart-2/50-foreground hover:opacity-90 px-10 shadow-lg shadow-chart-2/50/20 font-medium transition-all active:scale-95"
                >
                  Xác nhận
                </Button>
              </div>
